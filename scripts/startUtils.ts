import config, {ClusterConfig, AnalyzerConfig, DBMonitorConfig, DBOptions} from "@darcher/config";
...
export function startCluster(darcher: AnalyzerConfig, cluster: ClusterConfig) {
    if (!testAndMakeDir(cluster.dir)) {
        console.error(`mkdir ${cluster.dir} failed`);
        return false;
    }
    ...
    // start ethmonitor
    let ethmonitorCmd = `yarn workspace @darcher/go-ethereum start:ethmonitor --ethmonitor.port ${cluster.ethmonitorPort} --ethmonitor.controller ${cluster.controller} --analyzer.address localhost${darcher.grpcPort} --verbosity 4`;
    ...
    setTimeout(async () => {
        ...
        seg.push(`--ethmonitor.address localhost:${cluster.ethmonitorPort}`);
        if (cluster.extra) {
