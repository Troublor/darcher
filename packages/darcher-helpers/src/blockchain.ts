...
export class BlockchainCluster {
    ...
    public reset(): boolean {
        ...
        const doerDir = path.join(this.config.dir, "doer");
        if (!BlockchainCluster.testAndMakeDir(doerDir)) {
            console.error(`mkdir ${doerDir} failed`);
            return false;
        }
        const talkerDir = path.join(this.config.dir, "talker");
        ...
        // re-initialize doer
        const cmd1 = new Command("yarn workspace @darcher/go-ethereum start:geth");
        ...
        const cmd2 = new Command("yarn workspace @darcher/go-ethereum start:geth");
     ...
     */
    public async deploy(background = false): Promise<boolean | { doerPID: number, talkerPID: number }> {
        if (!BlockchainCluster.testAndMakeDir(this.config.dir)) {
            console.error(`mkdir ${this.config.dir} failed`);
            return false;
        }
        ...
        const cmdDoer = new Command(GETH_PATH);
        ...
        const cmdTalker = new Command(GETH_PATH);
        ...
        const talkerHttpPort = await getPort({port: getPort.makeRange(30300, 30399)});
        ...
        if (background) {
            ...
            const doerProcess = spawn(cmdDoer.command, cmdDoer.args, {
                stdio: ["ignore", "ignore", "ignore"],
                detached: true,
            });
            doerProcess.unref();
            const talkerProcess = spawn(cmdTalker.command, cmdTalker.args, {
                stdio: ["ignore", "ignore", "ignore"],
                detached: true,
            });
            talkerProcess.unref();
            ...
            const tab1 = new Tab(cmdDoer, false, undefined, `DOER-${this.config.ethmonitorPort}`);
            const tab2 = new Tab(cmdTalker, false, undefined, `TALKER-${this.config.ethmonitorPort}`);
            // open them three in a new Terminal window
            const window = new TerminalWindow(tab1, tab2);
            window.open();
            returned = true;
        }
        ...
        const nodeInfo = resp.data as { result: { enode: string } };
     ...
     * @return if start in background, return value will be a promise of PIDs of ethmonitor, doer and talker; otherwise return promise of boolean.
     */
    public async start(background = false): Promise<boolean | { ethmonitorPID: number, doerPID: number, talkerPID: number }> {
        if (!BlockchainCluster.testAndMakeDir(this.config.dir)) {
            console.error(`mkdir ${this.config.dir} failed`);
            return false;
        }
        ...
        const cmdEthmonitor = new Command(ETHMONITOR_PATH);
        ...
        if (background) {
            // open in the background
            const ethmonitorProcess = spawn(cmdEthmonitor.command, cmdEthmonitor.args, {
                detached: true,
            });
            ...
            const tab0 = new Tab(cmdEthmonitor, false, undefined, `Ethmonitor-${this.config.ethmonitorPort}`);
            const tab1 = new Tab(cmdDoer, false, undefined, `DOER-${this.config.ethmonitorPort}`);
            const tab2 = new Tab(cmdTalker, false, undefined, `TALKER-${this.config.ethmonitorPort}`);
            // open them three in a new Terminal window
            const window = new TerminalWindow(tab0, tab1, tab2);
            window.open();
     ...
     */
    public async stop() {
        for (const pid of [this.ethmonitorPID, this.doerPID, this.talkerPID]) {
            if (pid) {
...
export function startDarcher(darcher: AnalyzerConfig, dbMonitor: DBMonitorConfig, configFile: string) {
    const seg = ["yarn workspace @darcher/analyzer start:darcher"];
...
export function startDBMonitor(darcher: AnalyzerConfig, dbMonitor: DBMonitorConfig) {
    if (dbMonitor.db === DBOptions.indexedDB) {
        ...
        const seg = ["yarn workspace @darcher/dbmonitor-browser watch"];
...
export function getTruffleDeployedContract(truffleJsonFilePath: string): { contractName: string, abi: object, deployment: { [networkId: number]: { address: string } } } {
    const obj = JSON.parse(fs.readFileSync(truffleJsonFilePath, "utf8"));
    if (obj == null) {
        throw new Error(`${truffleJsonFilePath} is not a valid json file`);
    }
    // eslint-disable-next-line @typescript-eslint/ban-types
    const ret: { contractName: string, abi: object, deployment: { [networkId: number]: { address: string } } } = {
        contractName: "",
        abi: null,
        deployment: {},
    };
