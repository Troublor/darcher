...
export default class SimpleTimeLimitHook extends BaseHook {
    ...
    beforeCallM(context: Context, obj: object, prop: any, args: any[]): [object, any, any[]] {
        ...
        if (SimpleTimeLimitHook.isFeathersService(obj) && prop === "hook") {
        ...
        if (!this.globalHookAdded && SimpleTimeLimitHook.isFeathersApp(obj)) {
                        ...
                        before: [
                            ...
                            async (context) => {
                                switch (context.method) {
                                    ...
                                    case "patch":
                                    case "remove":
                                        console.log("update/patch event, try to fetch From: ", SimpleTimeLimitHook.getFeathersServiceName(context), context.id, context.params);
                                        if (context.id !== null && context.id !== undefined) {
                                            if (context.params.query)
                                                delete context.params.query.$select;
                                            // @ts-ignore
        ...
        if (SimpleTimeLimitHook.isFeathersService(obj)) {
            ...
            switch (prop) {
                case "create":
                    service.once("created", async (data, ctx: HookContext) => {
                        ...
                        dArcherNotifier.notifyDAppStateChange(
                            SimpleTimeLimitHook.genDAppStateMsg(
                                txState, txHash, SimpleTimeLimitHook.getFeathersServiceName(ctx), null, data, CRUDOperation.CREATE
                            ),
                            (err: any, resp: Reply) => {
                                if (resp.Err !== null) {
                ...
                case "remove":
                    service.once("removed", async (data, ctx: HookContext) => {
                        ...
                        dArcherNotifier.notifyDAppStateChange(
                            SimpleTimeLimitHook.genDAppStateMsg(
                                txState, txHash, SimpleTimeLimitHook.getFeathersServiceName(ctx), from, null, CRUDOperation.DELETE
                            ),
                            (err: any, resp: Reply) => {
                                if (resp.Err !== null) {
                                    console.error(resp.Err);
                ...
                case "patch":
                    service.once("patched", async (data, ctx: HookContext) => {
                        ...
                        dArcherNotifier.notifyDAppStateChange(
                            SimpleTimeLimitHook.genDAppStateMsg(
                                txState, txHash, SimpleTimeLimitHook.getFeathersServiceName(ctx), from, to, CRUDOperation.UPDATE
                            ),
                            (err: any, resp: Reply) => {
                                if (resp.Err !== null) {
                                    console.error(resp.Err);
    ...
    onStartFn(context: Context, args: any[], argList: IArguments): void {
        ...
        Recognizer.checkConfirmationCallback(argList, (confirmationNumber, receipt) => {
            ...
            this.currentTxState = confirmationNumber >= SimpleTimeLimitHook.confirmationsCount ? LifecycleState.CONFIRMED : LifecycleState.EXECUTED;
            this.setProcessTimeout();
        });
    ...
    private setProcessTimeout() {
        ...
        }, SimpleTimeLimitHook.processTimeout);
    }
