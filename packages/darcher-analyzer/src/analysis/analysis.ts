...
export class TxAnalysis {
    ...
    constructor(txHash: string) {
        ...
        this.txStateEntries = {};
        this.txStateEntries[TxState.CREATED] = [];
        this.txStateEntries[TxState.PENDING] = [];
        this.txStateEntries[TxState.EXECUTED] = [];
        this.txStateEntries[TxState.REVERTED] = [];
        this.txStateEntries[TxState.REEXECUTED] = [];
        this.txStateEntries[TxState.CONFIRMED] = [];
    }

    public add(txState: TxState, msg: DAppStateChangeMsg): void {
    ...
    public analyze(): boolean {
        ...
        let records1 = stateTableSlices[TxState.EXECUTED].fromRecords;
        let records2 = stateTableSlices[TxState.REVERTED].toRecords;
        if (!TxAnalysis.isIdenticalRecords(records1, records2)) {
            return false;
        }

        // check after executed dapp state and after re-executed dapp state
        records1 = stateTableSlices[TxState.EXECUTED].toRecords;
        records2 = stateTableSlices[TxState.REEXECUTED].toRecords;
        if (!TxAnalysis.isIdenticalRecords(records1, records2)) {
            return false;
        }
        return true;
