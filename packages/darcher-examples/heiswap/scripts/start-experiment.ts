...
if (require.main === module) {
    (async () => {
        const logger = new Logger("HeiswapExperiment", "debug");
        const config = await loadConfig(path.join(__dirname, "config", "heiswap.config.ts"));
        const mainClass: string = "HeiswapExperiment";
