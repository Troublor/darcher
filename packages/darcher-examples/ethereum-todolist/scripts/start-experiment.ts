...
if (require.main === module) {
    (async () => {
        const logger = new Logger("TodolistExperiment", "debug");
        const config = await loadConfig(path.join(__dirname, "config", "ethereum-todolist.config.ts"));
        const mainClass: string = "TodolistExperiment";
        ...
        async function oneRound(budget: number) {
            ...
            const time = config.analyzer.txStateChangeProcessTime ? config.analyzer.txStateChangeProcessTime : 15000;
            await sleep(time * 5);
