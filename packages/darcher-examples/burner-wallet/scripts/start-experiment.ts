...
if (require.main === module) {
    (async () => {
        const logger = new Logger("BurnerWalletExperiment", "debug");
        const config = await loadConfig(path.join(__dirname, "config", "burner-wallet.config.ts"));
        const mainClass: string = "BurnerWalletExperiment";
        ...
        async function oneRound(budget: number) {
            ...
            await clearLocalStorage(dappUrl, browser.driver);
