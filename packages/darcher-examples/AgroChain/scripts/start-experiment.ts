...
if (require.main === module) {
    (async () => {
        const logger = new Logger("AgroChainExperiment", "debug");
        const config = await loadConfig(path.join(__dirname, "config", "AgroChain.config.ts"));
        const mainClass: string = "AgroChainExperiment";
