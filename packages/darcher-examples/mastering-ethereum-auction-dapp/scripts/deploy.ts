...
async function deploy(config: Config) {
    ...
    child_process.spawnSync(
        ...
        ["migrate", "--reset"],{
            stdio: "inherit",
            cwd: path.join(__dirname, "..", "mastering-ethereum-auction-dapp", "backend")
        }
    );
...
if (require.main === module) {
    loadConfig(path.join(__dirname, "config", "mastering-ethereum-auction-dapp.config.ts")).then(async config => {
        await deploy(config);
    });
}
