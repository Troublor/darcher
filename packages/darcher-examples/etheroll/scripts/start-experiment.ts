...
if (require.main === module) {
    (async () => {
        const logger = new Logger("EtherollExperiment", "debug");
        const config = await loadConfig(path.join(__dirname, "config", "etheroll.config.ts"));
        const mainClass: string = "EtherollExperiment";
